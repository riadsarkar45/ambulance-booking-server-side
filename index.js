...
async function run() {
    try {
        ...
        app.delete('/api/delete/:id/:type', async (req, res) => {
            ...
            if (type === "ambulance") {
                const query = { _id: new ObjectId(id) }
                const ambulances = await ambulanceDetails.deleteOne(query)
                res.send(ambulances)
            } else if (type === "user") {
                const query = { _id: new ObjectId(id) }
                ...
                const query = { _id: new ObjectId(id) }
                const booking = await bookings.deleteOne(query)
                res.send(booking)
            }else if (type === "request"){
                const query = { _id: new ObjectId(id) }
                const requests = await patientRequest.deleteOne(query)
                res.send(requests)
            }else {
